...
-   Aggregation functions such as `sum()` and `median()` obey the usual rule of missing values:\
...
if (!requireNamespace("d3treeR", quietly = TRUE)) {
  message("Package 'd3treeR' is not installed. Run the installation commands above in the R Console.")
} else {
  library(d3treeR)
}
```
...
if (!requireNamespace("treemap", quietly = TRUE)) {
  message("Package 'treemap' is not installed. Install it to generate the treemap object.")
} else {
  library(treemap)
...
if (!exists("tm")) {
  message("Treemap object 'tm' not found. Please ensure treemap() ran successfully.")
} else if (!requireNamespace("d3treeR", quietly = TRUE)) {
  message("Package 'd3treeR' is not installed. Install it to enable interactive treemap.")
} else {
  library(d3treeR)
