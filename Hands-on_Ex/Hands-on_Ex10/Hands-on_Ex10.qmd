...
The Coffee Chain data is also available in Microsoft Access `.mdb` format.\
In practice, this file can be imported into R by using the `RODBC` package in a compatible Windows environment.\
However, for simplicity, this hands-on exercise uses the provided `.rds` file directly.

## 10.3.3 Data Preparation

The code chunk below is used to read the pre-saved `CoffeeChain.rds` file into R.
